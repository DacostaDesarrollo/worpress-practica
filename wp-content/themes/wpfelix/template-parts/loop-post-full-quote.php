<?php
/**
 * Template part for displaying posts full layout with video format.
 *
 * @package WPFelix
 */
?>
<article id="post-<?php the_ID(); ?>" <?php post_class( 'post-full' ); ?>>
    ...
    <div class="entry-summary">
        <?php
            $quote = wpfelix_get_element_string();
            if ( ! empty( $quote ) )
            {
                printf( '<div class="entry-quote">%s</div>', $quote );
            }
            else
            {
            ...
            wpfelix_the_content_strip_first_shortcode( $quote, sprintf(
